...
class Player:
    ...
    def __init__(self, depth, evalFn, util):
        self.evalFn = evalFn
        self.depth = depth
        self.util = util
    ...
    def getAction(self, gameState):
        def V(gameState, depth, evalFn):
            ...
            if(self.util.isEnd(gameState)):
                ...
                return (evalFn(gameState,self.util.isEnd(gameState),self.util.getScore(gameState)), random.choice(legalMoves))
            else:
                #start = time.time()
                newStates = [self.util.generateSuccessor(action, gameState) for action in legalMoves]
                #mid = time.time()
                scores = []
                state_vals = [[evalFn(item,self.util.isEnd(item),self.util.getScore(item)) for item in lst] for lst in newStates]
